...
import type { AuthProvider, AuthResult, HIDConfig } from './interfaces';
...
export class HIDProvider implements AuthProvider {
  private readonly config: HIDConfig;
  private readonly store: WebStorageStateStore;
  private readonly userManager: Promise<UserManager>;

  public constructor(config: HIDConfig) {
    this.config = config;
  ...
  public clearSessionStorage = async (): Promise<void> => {
  ...
  public init = async (): Promise<AuthResult> => {
    ...
    const model = new LiveModel({
      ...
      hidToken: user?.access_token ?? null,
      clearSessionStorage: this.clearSessionStorage,
    });
