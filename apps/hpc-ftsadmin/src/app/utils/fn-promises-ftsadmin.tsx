import { categories, FormObjectValue, flows } from '@unocha/hpc-data';
...
export const fnProjects = async (
  ...
  return response.map((responseValue) => ({
    displayLabel: `${responseValue.name} [${responseValue.code}]`,
    value: responseValue.id,
  }));
};

export const fnReportDetails = async (
  query: { query: string },
  env: Environment
) => {
  const response =
...
export const fnGetCurrencies = async (
  query: { query: string },
  env: Environment
) => {
...
export const fnGetAutocompleteFlows = async (
  query: { query: string },
  env: Environment
) => {
